...
export default defineEventHandler(async (event) => {
  ...
  await enforceRateLimit(event, 'upstream-sub-proxy-test', 30, 60_000)
  const id = getRouterParam(event, 'id') || ''
  const result = await runUpstreamOperation(event, {
    adminId: admin.userId,
    connectionId: 'sub2api',
    action: 'sub.proxy.test',
    targetType: 'sub2api_proxy',
    targetRef: id,
    fingerprint: { id, at: Math.floor(Date.now() / 10_000) },
    safeSummary: { check: 'connectivity' }
  }, async () => ({ result: await testManagedSub2ApiProxy(event, id) }))
  await writeAudit(event, admin.userId, 'sub.proxy.test', 'sub2api_proxy', id, {
    ok: result.ok,
