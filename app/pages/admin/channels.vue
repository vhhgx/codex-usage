  ...
  <div class="admin-page">
    ...
    <div v-if="showForm" class="admin-modal-backdrop" @click.self="showForm = false"><section class="admin-modal admin-modal--wide" role="dialog" aria-modal="true">
      ...
      <form class="admin-form" @submit.prevent="save">
        ...
        <section class="form-section"><header><div><h3>上游协议</h3><span>同一渠道可以多选</span></div></header><div class="channel-protocol-picker"><button v-for="option in protocolOptions" :key="option.id" type="button" :class="{ active: selectedProtocol(option.id) }" @click="toggleProtocol(option.id)"><IconCheck :size="15" /><span><strong>{{ option.label }}</strong><small>{{ option.detail }}</small></span></button></div><div class="channel-compatibility"><span>Claude Code：{{ selectedProtocol('anthropic_messages') ? '原生' : selectedProtocol('openai_chat') ? '协议转换' : '不可用' }}</span><span>Codex：{{ selectedProtocol('openai_responses') ? '原生' : '不可用' }}</span></div></section>
...
@media (max-width:700px) { .channel-protocol-picker { grid-template-columns:1fr; } }
