  ...
  <div class="admin-page">
    ...
    <div v-if="showForm" class="admin-modal-backdrop" @click.self="showForm = false"><section class="admin-modal admin-modal--wide" role="dialog" aria-modal="true">
      ...
      <form class="admin-form" @submit.prevent="save">
        ...
        <section class="form-section channel-compat-options"><header><div><h3>兼容设置</h3><span>客户端限定站点建议手工模型并开启身份透传</span></div></header><label class="switch"><input v-model="form.modelDiscoveryEnabled" type="checkbox"><span />保存和健康检查时自动读取模型</label><label class="switch"><input v-model="form.clientIdentityMode" type="checkbox" true-value="passthrough" false-value="standard"><span />透传真实 Claude Code / Codex 客户端身份</label></section>
