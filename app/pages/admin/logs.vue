...
const query = computed(() => ({ page: page.value, pageSize: 50, search: search.value || undefined, status: status.value || undefined, model: model.value || undefined, endpoint: endpoint.value || undefined, keyId: keyId.value || undefined, channelId: channelId.value || undefined, from: from.value || undefined, to: to.value || undefined }))
const { data, refresh } = await useFetch<{ items: RequestLogView[]; page: number; pageSize: number; total: number }>('/api/admin/logs', { query })
...
watch([search, status, model, endpoint, keyId, channelId, from, to], () => { clearTimeout(timer); timer = setTimeout(() => { page.value = 1; refresh() }, 300) })
watch(page, () => refresh())
  ...
  <div class="admin-page admin-page--logs">
    ...
    <div v-if="detail || detailLoading || detailError" class="log-drawer-backdrop" @click.self="closeDetail"><aside class="log-drawer"><header><div><span>REQUEST DETAIL</span><code>{{ detail?.requestId || (detailError ? '加载失败' : '正在载入…') }}</code></div><button class="icon-button" @click="closeDetail"><IconX :size="18" /></button></header>
      ...
      <template v-if="detail">
        ...
        <section class="log-attempts"><h3>调度轨迹</h3><div v-for="attempt in detail.attempts" :key="attempt.id"><span>{{ attempt.attempt }}</span><strong>{{ attempt.status }}</strong><code>{{ attempt.httpStatus || '网络错误' }}</code><small>{{ attempt.durationMs }}ms</small><p v-if="attempt.errorMessage">{{ attempt.errorMessage }}</p></div></section>
      </template>
    </aside></div>
  </div>
</template>
