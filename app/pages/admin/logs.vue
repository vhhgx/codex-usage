...
import { IconChevronLeft, IconChevronRight, IconCode, IconRefresh, IconSearch, IconX } from '@tabler/icons-vue'
...
const { data, refresh, status: logsStatus } = await useFetch<{ items: RequestLogView[]; page: number; pageSize: number; total: number; resourceStats: ResourceStat[]; executionStats: ExecutionStat[] }>('/api/admin/logs', { query })
...
<template>
  <div class="admin-page admin-page--logs">
    <header class="admin-page__header"><div><span class="admin-kicker">REQUEST FORENSICS</span><h1>请求日志</h1><p>逐次查看模型、渠道、Token、耗时、错误和完整审计正文。</p></div><button class="button button--secondary button--small" :disabled="logsStatus === 'pending'" @click="refresh()"><IconRefresh :class="{ 'is-spinning': logsStatus === 'pending' }" :size="15" />刷新</button></header>
