  ...
  <div class="admin-page">
    ...
    <section class="admin-table-wrap">
      <table class="admin-table"><thead><tr><th>Key</th><th>用户 / 分组</th><th>状态</th><th>权限</th><th>速率</th><th>到期时间</th><th>最近使用</th><th aria-label="操作" /></tr></thead>
        <tbody><tr v-for="item in filtered" :key="item.id">
          ...
          <td><div class="table-actions"><button class="icon-button" title="查看完整 Key" :disabled="!item.revealable" @click="openSecret(item, 'reveal')"><IconEye :size="17" /></button><button class="icon-button" title="设置完整 Key" @click="openSecret(item, 'replace')"><IconKey :size="16" /></button><button class="icon-button" title="用量详情" @click="openUsage(item)"><IconChartBar :size="17" /></button><button class="icon-button" :title="item.status === 'active' ? '停用' : '启用'" @click="toggle(item)"><IconDotsVertical :size="17" /></button><button class="icon-button" title="编辑策略" @click="openEdit(item)"><IconEdit :size="16" /></button><button class="icon-button danger" title="删除" @click="remove(item)"><IconTrash :size="16" /></button></div></td>
    ...
    <div v-if="showForm" class="admin-modal-backdrop" @click.self="showForm = false">
      <section class="admin-modal admin-modal--wide" role="dialog" aria-modal="true">
        <header><div><span>HUB KEY</span><h2>{{ editing ? '编辑访问策略' : '创建访问凭据' }}</h2></div><button class="icon-button" title="关闭" @click="showForm = false"><IconX :size="18" /></button></header>
