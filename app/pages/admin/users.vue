...
<template>
  <div class="admin-page">
    ...
    <div v-if="detail" class="log-drawer-backdrop" @click.self="detail = null"><aside class="log-drawer key-usage-drawer"><header><div><span>USER DETAIL</span><code>{{ detail.user.username }}</code></div><button class="icon-button" title="关闭" aria-label="关闭" @click="detail = null"><IconX :size="18" /></button></header><section class="key-usage-identity"><span class="key-glyph"><IconUser :size="17" /></span><div><h2>{{ detail.user.displayName || detail.user.username }}</h2><p>{{ detail.user.groupNames.join('、') || '未分组' }}</p></div><span class="status-dot" :data-status="detail.user.status"><i />{{ detail.user.status }}</span></section><section class="admin-metrics"><article><span>请求</span><strong>{{ detail.usage.requests }}</strong></article><article><span>Token</span><strong>{{ formatTokenCount(detail.usage.tokens) }}</strong></article><article><span>成本</span><strong>${{ detail.usage.cost.toFixed(4) }}</strong></article></section><section class="key-credentials"><header><div><h3>Hub Keys</h3><span>{{ detail.keys.length }} 个</span></div></header><div v-for="key in detail.keys" :key="key.id" class="credential-row"><div><strong>{{ key.name }}</strong><code>{{ key.maskedKey }}</code></div><span class="status-dot" :data-status="key.status"><i />{{ key.status }}</span></div></section><section class="key-usage-recent"><header><h3>最近活动</h3><span>{{ detail.recent.length }} 条</span></header><div v-for="item in detail.recent" :key="item.id"><code>{{ item.requestId }}</code><span>{{ item.model || '—' }}</span><strong>{{ item.httpStatus || '—' }}</strong><small>{{ date(item.createdAt) }}</small></div><p v-if="!detail.recent.length">尚无请求记录</p></section></aside></div>
