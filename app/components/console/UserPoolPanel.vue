...
const { data: receiverData, refresh: refreshReceivers } = await useFetch<{ items: SmsReceiverView[] }>('/api/console/pool/sms-receivers')
const { data: vaultData, refresh: refreshVault } = await useFetch<{ items: AccountVaultView[] }>('/api/console/pool/account-vault')
const { data: usageData, refresh: refreshUsage } = await useFetch<{ items: Array<{ accountId: string; quotaStatus: string; planType: string | null; windows: Array<{ label: string; remainingPercent: number | null; remaining: number | null; resetAt: number | null }>; error?: string }>; generatedAt: number }>('/api/console/pool/usage')
const { data: planData } = await useFetch<{ subscription: null | { status: string; plan: { entitlementSnapshot?: Record<string, unknown>; version?: { supplyMode?: string; maxPoolAccounts?: number | null } | null } } }>('/api/console/plan')
