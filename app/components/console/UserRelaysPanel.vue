...
const { data, refresh } = await useFetch<{ groups: UserRelayGroupView[] }>('/api/console/relay-groups')
...
const visibleRelays = computed<UserRelayAccountView[]>(() => data.value?.groups.flatMap(group => { const account = activeGroup(group); return account ? [account] : [] }) || [])
...
<template>
  <div class="admin-page relay-page">
    ...
    <section v-if="data?.groups.length" class="relay-group-tabs">
      <article v-for="group in data.groups" :key="group.id" class="relay-group-summary">
        ...
        <nav class="relay-account-tabs" aria-label="中转账号"><div v-for="(account, index) in group.accounts" :key="account.id" class="relay-account-tab" :class="{ active: (activeAccountByGroup[group.id] || group.accounts[0]?.id) === account.id, dragging: draggedAccount?.accountId === account.id }" :draggable="group.accountOrderMode === 'manual'" @dragstart="startAccountDrag(group.id, account.id, $event)" @dragend="draggedAccount = null" @dragover.prevent @drop.prevent="dropAccount(group, account.id)"><IconGripVertical v-if="group.accountOrderMode === 'manual'" class="relay-drag-handle" :size="14" /><button type="button" @click="activeAccountByGroup[group.id] = account.id"><span>{{ account.accountLabel || account.name }}</span><small>{{ account.state.routingState === 'depleted' ? '额度耗尽，等待刷新' : account.state.routingState === 'credential_error' ? '凭据失效' : account.state.remainingBalance === null ? '余额待查' : formatBalance(account.state.remainingBalance, account.state.currency) }}</small></button><span v-if="group.accountOrderMode === 'manual'" class="relay-tab-order"><button type="button" title="前移" aria-label="账号前移" :disabled="index === 0" @click="moveAccount(group, account.id, -1)"><IconChevronLeft :size="13" /></button><button type="button" title="后移" aria-label="账号后移" :disabled="index === group.accounts.length - 1" @click="moveAccount(group, account.id, 1)"><IconChevronRight :size="13" /></button></span></div></nav>
        <article v-if="activeGroup(group)" class="relay-row">
          <div class="relay-identity"><span><IconServerBolt :size="19" /></span><div><strong>{{ activeGroup(group)!.name }}</strong><a class="relay-url" :href="activeGroup(group)!.baseUrl" target="_blank" rel="noopener noreferrer" :title="`打开 ${activeGroup(group)!.baseUrl}`"><IconExternalLink :size="12" />{{ activeGroup(group)!.baseUrl }}</a><small>仅自己 · {{ activeGroup(group)!.models.filter(model => model.enabled).length }} 个模型</small></div></div>
          <div class="relay-protocols"><span v-for="protocol in activeGroup(group)!.protocols" :key="protocol.id || protocol.protocol" :data-status="protocol.verificationStatus"><i />{{ protocolLabel(protocol.protocol) }}</span></div>
          <div class="relay-health"><strong>{{ activeGroup(group)!.healthStatus === 'healthy' ? '可用' : activeGroup(group)!.healthStatus === 'unhealthy' ? '需处理' : '待检测' }}</strong><small>{{ date(activeGroup(group)!.lastHealthCheckAt) }}</small><em v-if="activeGroup(group)!.lastHealthError" :title="activeGroup(group)!.lastHealthError || undefined">{{ activeGroup(group)!.lastHealthError }}</em></div>
          <div class="relay-balance"><template v-if="activeGroup(group)!.state.balanceError"><strong>查询失败</strong><small :title="activeGroup(group)!.state.balanceError || undefined">{{ activeGroup(group)!.state.balanceError }}</small></template><template v-else-if="activeGroup(group)!.state.balanceStatus === 'success'"><strong>{{ formatBalance(activeGroup(group)!.state.remainingBalance, activeGroup(group)!.state.currency) }}</strong><small>购买 {{ formatBalance(activeGroup(group)!.state.purchasedQuota, activeGroup(group)!.state.currency) }} · 赠送 {{ formatBalance(activeGroup(group)!.state.giftQuota, activeGroup(group)!.state.currency) }}</small></template><template v-else><strong>余额待查</strong><small>{{ group.platformType === 'generic' ? '通用站未配置余额接口' : '点击刷新余额' }}</small></template></div>
          <div class="table-actions"><button class="button button--quiet button--small relay-model-button" :disabled="!activeGroup(group)!.models.length" @click="openModels(activeGroup(group)!)"><IconBraces :size="14" />查看模型 <span>{{ activeGroup(group)!.models.filter(model => model.enabled).length }}</span></button><button class="icon-button" title="刷新余额" :aria-label="`${activeGroup(group)!.name} 刷新余额`" :disabled="balanceLoading === activeGroup(group)!.id || group.platformType === 'generic' || (group.platformType === 'newapi' && !activeGroup(group)!.checkinConfigured)" @click="refreshBalance(activeGroup(group)!)"><IconWallet :size="16" /></button><button v-if="activeGroup(group)!.checkinEnabled && activeGroup(group)!.checkinConfigured" class="icon-button" :class="{ 'is-complete': checkedInToday(activeGroup(group)!) }" :title="checkedInToday(activeGroup(group)!) ? '今日已签到' : '签到'" :aria-label="`${activeGroup(group)!.name} ${checkedInToday(activeGroup(group)!) ? '今日已签到' : '签到'}`" :disabled="checkingIn === activeGroup(group)!.id || checkedInToday(activeGroup(group)!)" @click="checkin(activeGroup(group)!)"><IconCalendarCheck :size="17" /></button><button class="icon-button" :title="testing === activeGroup(group)!.id ? '协议检测正在执行' : '协议检测'" :aria-label="testing === activeGroup(group)!.id ? `${activeGroup(group)!.name} 协议检测正在执行` : `${activeGroup(group)!.name} 协议检测`" :aria-busy="testing === activeGroup(group)!.id" @click="requestTest(activeGroup(group)!)"><IconRefresh :class="{ 'is-spinning': testing === activeGroup(group)!.id }" :size="17" /></button><button class="icon-button" title="同步模型" aria-label="同步模型" :disabled="syncing === activeGroup(group)!.id" @click="sync(activeGroup(group)!)"><IconCloudDownload :size="17" /></button><button class="icon-button" title="复制中转" aria-label="复制中转" @click="openDuplicate(activeGroup(group)!)"><IconCopy :size="17" /></button><button class="icon-button" title="生成客户端配置" aria-label="生成客户端配置" @click="openConfig(activeGroup(group)!)"><IconCode :size="17" /></button><button class="icon-button" title="编辑" aria-label="编辑中转" @click="edit(activeGroup(group)!)"><IconEdit :size="17" /></button><button class="icon-button danger" title="删除" aria-label="删除中转" @click="deleting = activeGroup(group)!"><IconTrash :size="17" /></button></div>
        </article>
      </article>
    </section>
    <div v-if="!data?.groups.length" class="admin-empty relay-empty"><IconServerBolt :size="26" /><strong>还没有私有中转</strong><p>添加一个支持 Messages、Responses 或 Chat 的站点。</p><button class="button button--primary button--small" @click="create()">添加第一个站点</button></div>
